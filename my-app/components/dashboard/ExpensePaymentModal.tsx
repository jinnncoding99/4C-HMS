// components/dashboard/ExpensePaymentModal.tsx
'use client';
    ...
    try {
      ...
      if (isDirectSettlement) {
        ...
        const { error: historyError } = await supabase.from('transaction_history').insert([{
          ...
          total_amount: expense.total_amount ?? expense.amount ?? parsedAmount,
          billing_period_start: expense.billing_period_start || null,
          billing_period_end: expense.billing_period_end || null,
          calculation_type: expense.calculation_type || null,
          settled_at: new Date().toISOString(),
          url_receipt: receiptUrl,
          payment_receiver: receiverProfile?.username || null,
          payment_receiver_id: expense.payment_receiver_id || receiverProfile?.id || null
        }]);
        ...
        if (deleteExpenseError) {
          console.error("expenses delete failed:", deleteExpenseError);
          throw new Error(`Database cleanup failed: ${deleteExpenseError.message || JSON.stringify(deleteExpenseError)}`);
        }
        ...
        if (shareError) {
          console.error("expense_shares pending update failed:", shareError);
          throw new Error(`Expense share update failed: ${shareError.message || JSON.stringify(shareError)}`);
        }
      ...
      <div className="bg-white dark:bg-[#18181b] p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 w-full max-w-md max-h-[85vh] overflow-y-auto space-y-4 text-slate-800 dark:text-zinc-100 shadow-2xl pb-12">
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">
            {isDirectSettlement ? "Settle Expense Directly" : "Submit Payment"}
          </h3>
          {isDirectSettlement && (
        ...
        <div className="flex gap-2 pt-2">
          ...
          >
            {submitting ? "Processing..." : isScanning ? "Verifying..." : isDirectSettlement ? "Settle & Clear Expense" : "Confirm & Submit"}
          </Button>
          <Button 
            variant="ghost" 
            onClick={onClose} 
