...
export default function ExpensePaymentModal({ expense, initialAmount, receiverProfile, onClose, onSuccess }: ExpensePaymentModalProps) {
  ...
  const handleFileChange = async (file: File | null) => {
    ...
    try {
      ...
      if (result.isValid) {
        ...
        // If OCR validation fails, nullify receiptFile so it's treated as invalid
        setReceiptFile(null);
  ...
  const submitPaymentRequest = async () => {
    ...
    try {
      ...
      if (paymentMethod === 'online' && receiptFile) {
        ...
        receiptUrl = "not applicable";
      }

      const notificationPayload = {
        type: 'payment_approval',
        email: user.email || 'User',
        message: `Payment submitted for expense: ${expense.description} via ${paymentMethod.toUpperCase()} amount: ₱${Number(paymentAmount).toFixed(2)}`,
        status: 'pending',
        receiver_id: receiverProfile?.id || null,
        details: {
      ...
      console.error("Error submitting expense payment:", errorMessage);
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[9999]">
      <div className="bg-[#1a1a1a] p-6 rounded-2xl border border-[#ff8c00] w-full max-w-md max-h-[85vh] overflow-y-auto space-y-4 text-white shadow-2xl pb-12">
        <h3 className="text-lg font-bold">Submit Expense Payment</h3>
        ...
        <div className="bg-[#111] p-3 rounded-xl border border-[#333] text-center space-y-2">
          ...
          <p className="font-bold text-[#ff8c00] text-base">{receiverProfile?.username || 'Designated Receiver'}</p>
          
          {receiverProfile?.qr_code_url ? (
            <img 
              src={receiverProfile.qr_code_url} 
              alt="Receiver QR Code" 
              className="w-32 h-32 mx-auto rounded-lg border border-[#ff8c00] object-contain bg-white p-1" 
            />
          ) : (
        ...
        <div className="space-y-1">
          <label className="text-xs text-gray-300">Amount to Pay</label>
          <Input 
            type="number"
            step="0.01"
            value={paymentAmount}
            onChange={(e) => setPaymentAmount(e.target.value)}
            className="bg-[#111] border-[#333] text-white focus:border-[#ff8c00]"
          />
        </div>
        ...
        <div className="flex gap-2 pt-2">
          ...
          >
            {submitting ? "Submitting..." : isScanning ? "Verifying..." : "Submit Payment"}
          </Button>
          <Button 
            variant="ghost" 
            onClick={onClose} 
